...
namespace App.Aes
{
    ...
    {
        private readonly Algorithms.IAes _algorithm = new Algorithms.Aes();
        ...
        private void EncryptButton_Click(object sender, EventArgs e)
        {
            ...
            {
                string result = _algorithm.Encrypt(
                    PlaintextTextBox.Text, KeyTextBox.Text);

                if (result.Contains("FormatException"))
                {
                    PlaintextTextBox.BackColor = Color.Red;
                    KeyTextBox.BackColor = Color.Red;
        ...
        private void DecryptButton_Click(object sender, EventArgs e)
        {
            ...
            {
                string result = _algorithm.Decrypt(
                        CiphertextTextBox.Text, KeyTextBox.Text);

                bool resultIsEmpty = result.Length == 0;

                if (result.Contains("FormatException") || resultIsEmpty)
                {
                    CiphertextTextBox.BackColor = Color.Red;
                    KeyTextBox.BackColor = Color.Red;
                    ...
                    MessageBox.Show(
                        ...
                        (resultIsEmpty? string.Empty : " Reason: " + result),
                        "Wrong Input",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Exclamation);
